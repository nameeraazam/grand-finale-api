...
function ForyouCard({
    ...
    reviewStars,
    reviewCount,
    offer = '',  // Default empty string if no offer is provided
    classes = '', // Default empty string if no custom class is provided
}: Products) {
  return (
    <div className="flex flex-col gap-3">
        <div className="flex justify-center items-center w-[245px] h-[225px] bg-[#f5f5f5] relative overflow-y-hidden">
            {/* Display offer if it's provided */}
            ...
            <Image className="-mt-8 w-[150px] h-auto" src={image} alt={productAlt || 'Product Image'} />
        </div>
        <p>{title}</p>
        ...
        <p className="flex gap-3 items-center">
            <span className="flex">
                {Array(reviewStars).fill(<span className="text-yellow-400"><FaStar /></span>)}
            </span>
